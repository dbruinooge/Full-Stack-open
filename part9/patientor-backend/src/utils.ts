...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const isDiagnosisCode = (code: any): code is Diagnosis => {
  return diagnoses.some(diagnosis => diagnosis.code === code);
};

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const isDiagnosisCodes = (codes: any): codes is Diagnosis[] => {
  return (Array.isArray(codes) && codes.every(code => isDiagnosisCode(code)));
};

const parseDiagnosisCodes = (codes: unknown): Diagnosis[] => {
...
//   { type, description, date, specialist } : NewEntry,
//   id: string
// ): NewEntry => {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
// const isHealthCareRating = (param: any): param is HealthCheckRating => {
//   // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
//   return Object.values(HealthCheckRating).includes(param);
// };

// const parseHealthCareRating = (param: unknown): HealthCheckRating => {
//   if (!isHealthCareRating(param)) {
//     throw new Error ('Incorrect health care rating: ' + param);
//   }
  ...
  switch (newEntry.type) {
    ...
    case 'OccupationalHealthcare':
      if (!newEntry.employerName) {
        throw new Error ('Employer or sick leave missing');
      }
